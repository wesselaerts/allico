...
- `mcx_quiz.html` — de hele quiz app (~130 KB)
...
1. Zet beide bestanden in dezelfde map.
2. Dubbelklik `mcx_quiz.html` — opent in je browser.
...
3. **Upload de bestanden**:
   ...
   - Sleep `mcx_quiz.html` en `MCXAP64M96FS3RM.pdf` erin.
...
5. **Wacht ~30 seconden**, ververs de Pages instellingen-pagina. Je krijgt een URL te zien zoals:
   ```
   https://JOUW-USERNAME.github.io/mcx-quiz/mcx_quiz.html
   ```
6. **Open die URL** op je iPhone in Safari. De PDF wordt automatisch geladen (eerste keer ~10 sec, daarna gecached).
7. **Voeg toe aan beginscherm**: in Safari klik op het deel-icoontje (vierkant met pijl omhoog) → "Voeg toe aan beginscherm". Nu heb je een app-icoontje.

### iPhone tips
- Het paneel wordt full-screen op kleine schermen.
...
- Niets van dit alles staat in de HTML/PDF bestanden die op GitHub komen — die zijn statisch en bevatten geen persoonlijke data.
